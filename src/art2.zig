...
pub fn ArtTree(comptime T: type) type {
    return extern struct {
        ...
        fn recursiveInsert(t: *Tree, n: *Node, ref: **Node, key: []const u8, value: T, depth: u32) Error!Result {
            ...
            if (base_node.partial_len != 0) {
                ...
                if (prefix_diff >= base_node.partial_len) {
                    return try t.recursiveInsertSearch(n, ref, key, value, depth + base_node.partial_len);
                }
        ...
        fn recursiveDelete(t: *Tree, n: *Node, ref: **Node, key: []const u8, _depth: usize) Error!Result {
            ...
            const baseNode = n.baseNode();
            if (baseNode.partial_len > 0) {
                const prefix_len = checkPrefix(baseNode, key, depth);
                if (prefix_len != math.min(MaxPrefixLen, baseNode.partial_len))
                    return .missing;
                depth += baseNode.partial_len;
            }
        ...
        fn removeChild4(t: *Tree, n: *Node, ref: **Node, l: **Node) void {
            ...
            const baseNode = n.baseNode();
            const shift_len = baseNode.num_children - 1 - pos;
            mem.copy(u8, n.node4.keys[pos..], n.node4.keys[pos + 1 ..][0..shift_len]);
            mem.copy(*Node, n.node4.children[pos..], n.node4.children[pos + 1 ..][0..shift_len]);
            baseNode.num_children -= 1;
            // Remove nodes with only a single child
            if (baseNode.num_children == 1) {
                const child = n.node4.children[0];
                if (child.* != .leaf) {
                    // Concatenate the prefixes
                    var prefix = baseNode.partial_len;
                    if (prefix < MaxPrefixLen) {
                        baseNode.partial[prefix] = n.node4.keys[0];
                        prefix += 1;
                    }
                    const childBaseNode = child.baseNode();
                    if (prefix < MaxPrefixLen) {
                        const sub_prefix = math.min(childBaseNode.partial_len, MaxPrefixLen - prefix);
                        mem.copy(u8, baseNode.partial[prefix..], childBaseNode.partial[0..sub_prefix]);
                        prefix += sub_prefix;
                    }
                    mem.copy(u8, &childBaseNode.partial, baseNode.partial[0..math.min(prefix, MaxPrefixLen)]);
                    childBaseNode.partial_len += baseNode.partial_len + 1;
                }
                ref.* = child;
                t.a.destroy(n);
            }
        ...
        fn removeChild16(t: *Tree, n: *Node, ref: **Node, l: **Node) Error!void {
            ...
            const baseNode = n.baseNode();
            const shift_len = baseNode.num_children - 1 - pos;
            mem.copy(u8, n.node16.keys[pos..], n.node16.keys[pos + 1 ..][0..shift_len]);
            mem.copy(*Node, n.node16.children[pos..], n.node16.children[pos + 1 ..][0..shift_len]);
            baseNode.num_children -= 1;
            if (baseNode.num_children == 3) {
                const new_node = try t.allocNode(.node4);
                ref.* = new_node;
                copyHeader(new_node.baseNode(), baseNode);
                mem.copy(u8, &new_node.node4.keys, n.node16.keys[0..4]);
                mem.copy(*Node, &new_node.node4.children, n.node16.children[0..4]);
                t.a.destroy(n);
            }
        }
        fn removeChild48(t: *Tree, n: *Node, ref: **Node, c: u8) Error!void {
            ...
            if (baseNode.num_children == 12) {
                ...
                while (true) : (i += 1) {
                    pos = n.node48.keys[i];
                    if (pos != 0) {
                        new_node.node16.keys[child] = i;
                        new_node.node16.children[child] = n.node48.children[pos - 1];
                        child += 1;
                    }
                    if (i == 255) break;
                }
